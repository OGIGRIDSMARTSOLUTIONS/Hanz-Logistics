  ...
  return <>
    ...
    <main>
      ...
      <section className="industries" id="industries">
        ...
        <div className="industry-list section-pad" role="list">{industries.map(({icon:Icon,title,summary}, index) => <button type="button" role="listitem" className={activeIndustry === index ? 'active' : ''} key={title} onClick={() => setActiveIndustry(index)} onMouseEnter={() => setActiveIndustry(index)} aria-pressed={activeIndustry === index}><b>{String(index+1).padStart(2,'0')}</b><span className="industry-icon"><Icon /></span><h3>{title}</h3><p>{summary}</p><ArrowRight /></button>)}</div>
      </section>
